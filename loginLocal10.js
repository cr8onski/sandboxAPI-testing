...
var reqSalt = function () {
	...
	request({url:root + sandbox + vwf + 'salt?UID=' + UID}, reqLogin(error, resp, salt));
}
...
var reqLogin = function (error, response, salt) {
	if (!err && (response.statusCode === 200)) {
		var pw = EncryptPassword(pword, UID, salt);
		//Create form-data
		var formData = {
			username : UID,
			password : pw
		...
		request(options, reqLoginData(error, response, body));
	} else {
